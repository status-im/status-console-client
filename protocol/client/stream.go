...
type StreamHandler func(*protocol.StatusMessage) error
...
func StreamStoreHandlerMultiplexed(db Database) StreamHandler {
	return func(m *protocol.StatusMessage) error {
		return streamStoreHandlerMultiplexed(db, m)
	}
}

func streamStoreHandlerMultiplexed(db Database, sm *protocol.StatusMessage) error {
...
func (s *Stream) Start(aCtx context.Context, options protocol.SubscribeOptions) (err error) {
	...
	msgs := make(chan *protocol.StatusMessage, 100)
...
func (s *Stream) processLoop(ctx context.Context, messages <-chan *protocol.StatusMessage) {
	for {
		select {
		case msg := <-messages:
			err := s.handler(msg)
...
func (s *Stream) processHandlerErr(err error, msg *protocol.StatusMessage) {
	if err == ErrMsgAlreadyExist {
