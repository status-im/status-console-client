...
func (w *Waku) addAndBridge(envelope *common.Envelope, isP2P bool, bridged bool) (bool, error) {
	...
	w.logger.Info("SIZE", zap.Uint32("size", uint32(envelope.Size())))

	common.EnvelopesReceivedCounter.Inc()
	if sent > now {
		w.logger.Info("SENT NOW")
	...
	if envelope.PoW() < w.MinPow() {
		w.logger.Info("POW", zap.Float64("pow", envelope.PoW()), zap.Float64("min pow", w.MinPow()))
		// maybe the value was recently changed, and the peers did not adjust yet.
